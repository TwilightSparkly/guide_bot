...
func (b *Broker) NewBroker() error {
	...
	b.conn, b.err = amqp.Dial("amqp://guest:guest@localhost:5672/") // убрать в конфиг
	if b.err != nil {
		b.Log.LogErr.Println("NewBroker(): Unable to connect over TCP, error:", b.err)
		return b.err
	} else {
		b.Log.LogInfo.Println("NewBroker(): Connected to: amqp://guest:guest@localhost:5672/")
	}
...
func (b *Broker) Consume(qname string, ch chan UserMsg) error {
	...
	messages, b.err = b.ch.Consume( // убрать в отдельный метод и конфиг
